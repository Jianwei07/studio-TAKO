...
export const caseStudy = {
  ...
  description:
    'Find love in the face of fear — connectKind is a dating app that matches users based on their mutual phobias so they can be scared together.',
  summary: [
    'Find love in the face of fear — connectKind is a dating app that matches users based on their mutual phobias so they can be scared together.',
    'We worked with connectKind to develop a new onboarding flow. A user is shown pictures of common phobias and we use the microphone to detect which ones make them scream, feeding the results into the matching algorithm.',
  ],
  logo,
  image: { src: imageHero },
  date: '2022-06',
  ...
  testimonial: {
    author: { name: 'Jenny Wilson', role: 'CPO of connectKind' },
    content:
...
Noticing incredibly high churn, the team at connectKind came to the conclusion that, instead of having a fundamentally flawed business idea, they needed to improve their onboarding process.
